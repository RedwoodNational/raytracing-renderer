...
def from_img(src, dst):
	...
	for j in range(h):
		for i in range(w):
			r = pix[i, j][0].to_bytes(1, byteorder='big', signed=False)
			g = pix[i, j][1].to_bytes(1, byteorder='big', signed=False)
			b = pix[i, j][2].to_bytes(1, byteorder='big', signed=False)
			struct.pack_into('cccc', buff, offset, r, g, b, b'\0')
			offset += 4;
	out = open(dst, 'wb')
